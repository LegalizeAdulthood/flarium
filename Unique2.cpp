...
cmplx CFlarium24View::ParseExpression( const char* str, int& index )
{
	try
	{
		//
		...
		//
		cmplx Operand1 = ParseOperand( str, index );
		...
		while ( ( Operation != '\0' ) && ( Operation != ')' ) )
		{
			...
			//
			cmplx Operand2 = ParseOperand( str, ++index );
...
cmplx CFlarium24View::ParseOperand( const char* str, int& index )
{
	try
	{
		...
		switch ( str[ index ] )
		{
			...
			{
				index++;
				cmplx d = ParseExpression( str, index );
			...
			case 'C':
				...
				{
					index += 3;
          return ((ParseOperand( str, index )).ccos());
				}
        else 
        {
          index++;						// advance to the next character
			...
			case '.':
			{
				cmplx d = cmplx( atof( &( str[ index ] )), 0 );
			...
			default:
				...
				if ( strncmp( &( str[ index ] ), "sin(", 4 ) == 0 )
				{
					...
					return ((ParseOperand( str, index )).csin());
				}
				...
				{
					index += 4;
					return sinh( (cmplx&) ParseOperand( str, index ) );
				...
				{
					index += 4;
					return asin( (cmplx&) ParseOperand( str, index ) );
				}
				...
				{
					index += 4;
					return acos( (cmplx&) ParseOperand( str, index ) );
				}
				...
				{
					index += 3;
					return tangent( (cmplx&) ParseOperand( str, index ) );
				}
				...
				{
					index += 4;
					return arctan( (cmplx&) ParseOperand( str, index ) );
				}
				...
				{
					index += 3;
          return ((ParseOperand( str, index )).clog());
				}
...
void CFlarium24View::OnDrawUnique()
{
  ...
  if (cu.DoModal() == IDOK)
  {
    ...
    try
      ...
      z = ParseExpression( strFormulae, i_try );
...
void CFlarium24View::OnDraw140() 
{
	...
	ActiveTitle = "140) Unique Formulae";
	nDistortion = 140;
	GoDoFractal();
}

void CFlarium24View::OnUpdateDraw140(CCmdUI* pCmdUI) 
{
	if (nDistortion == 140) pCmdUI->SetCheck(TRUE);	
